...
\subsubsection{Kaltura} \label{subsubsec:analyza_reseni_kaltura}
TODO


\subsection{Technologie pro frontend} \label{subsec:analyza_reseni_frontend}
Stejně jako v úvodu technologií pro backend (\ref{subsec:analyza_reseni_backend}) je při rešerši žádoucí respektovat frontendové technologie dohodnuté nefunkčními požadavky (\ref{subsec:analyza_np}). V tomto případě je nefunkčním požadavkem N7 specifikován výčet tří technologií, které jsou framework Nette jazyku PHP, dále značkovací jazyk HTML5 a skriptovací jazyk JavaScript. Technologie HTML5 i JavaScript jsou nativní technologie pro vývoj frontendu webových aplikací a v tomto kontextu se tedy nedají brát jako komplexní řešení pro nějakou podmnožinu specifikovaných úkolů a není tedy důvod je zde analyzovat a porovnávat s ostatními. Dokonce ani framework Nette není třeba zahrnout do porovnání technologií, protože je v něm napsán téměř celý systém předchozí bakalářské práce a bude tedy použit i nadále.
...
\subsubsection{} \label{subsubsec:analyza_reseni_}
TODO

TODO JESTE ASPON DVE
...
Po porovnání všech nashromážděných technologií pomocí specifikovaných charakteristik bylo dosaženo výsledku, který je popsán tabulkou TODO NUM TABLE pro backendové technologie a tabulkou TODO NUM TABLE pro frontendové technologie.


	Jak již bylo zmíněno na začátku sekce o existujících řešeních (\ref{sec:analyza_existujici_reseni}), tak z důvodu úspory místa byly zde v práci analyzovány pouze takové technologie, které měly již předem potenciál dobrého vyhodnocení v tomto porovnání. Ovšem, v tabulce jsou již zmíněna i jména technologií, které byly taktéž analyzovány, avšak postup analýzy již v rešeršní části práce zmíněn nebyl, neboť jejich hodnocení dopadlo hůře.
