...
\subsection{UX a UI}
Tato sekce se věnuje návrhu a tvrobě UX (User eXperience) a UI (User Interface) frontendové části řešení ...
Se zužitkováním znalostí z (NUR pred) je využito ověřených postupů a je kladen důraz na principy dobrého UI jako jsou jednolitost, předpovídatelnost, zobrazování pouze užitečných informací, tolerance chyb a další. 
...
list all possible tasks, even the minor "obvious" ones (add record, delete record, add item to cart, change item quantity)
tasks can have various granularity and importance (at first, list all of them, then group them, sort importancy
