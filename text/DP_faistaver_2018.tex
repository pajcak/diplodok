...
\input{latex_html_style.tex} %syntax highlighting for HTML
\input{latex_json_style.tex} %syntax highlighting for JSON
...
Video.js je open-source knihovna pro práci s~videem na webu (vizte \cite{videojs}). Tato knihovna je vlastně obalení pro nativní webový videopřehrávač v~technologii HTML5, avšak s~přidanými užitečnými funkcionalitami. Jelikož je tento software open-source a~byl stavěný tak, aby byl dobře rozšiřitelný, tak se komunitě v~průběhu času podařilo vyvinout nemalý počet užitečných pluginů. Knihovna má výbornou dokumentaci, kde je mimo jiné i~popsáno, jak lze napsat vlastní plugin, což je zvládnutelné i~se základními znalostmi jazyku JavaScript. 
	
	Použití této knihovny je velice vhodné pro frontendovou část řešení této práce. Díky již zmíněným přednostem je lepší a~snadněji rozšiřitelná než samotný přehrávač vestavěný v~jazyku HTML5. Knihovnu je možné buď stáhnout a~v~případě potřeby i~upravit před použitím nebo použít CDN Fastly, která poskytuje hosting všem nezbytným souborům knihovny Video.js. Knihovna je kompatibilní s~prohlížeči se zabudovaným HTML5 nebo případně i~s~technologií Flash. Podporovanými prohlížeči je většina dnes nejčastěji používaných jako například Firefox verze 3.5 a~vyšší, Internet Explorer 6 a~vyšší, Chrome verze 3 a~vyšší, Opera a~další (více v~dokumentaci projektu).
...
Na tomto grafu je znázorněna jednoduchá konfigurace, kdy se berou data z~kamery (\uv{Capture device}), komprimovaná do MJPEG  se streamují do sítě (\uv{Encode MJPEG} a~\uv{Stream}) a~zároveň se ukládají na disk v~H264 (\uv{Encode H264} a~\uv{Save to HDD}).
\\
...
Uzel, který implementuje koncept \uv{BasicEventProducer}, může generovat dvojice jméno a~událost. Uzel sám neřeší, kam se událost posílá. Oproti tomu uzel, který implementuje koncept \uv{BasicEventConsumer} umí dvojice jméno a~událost přijímat. Jaká událost se kam posílá se specifikuje zvlášť v~konfiguraci.
...
\end{figure}
\begin{figure}[h]\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.02\textwidth]{images/ui_task_flow_2.eps}}
...
\begin{figure}\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.12\textwidth]{images/db_orig.eps}}
	\caption{Původní databázový model}\label{img:navrh_backend_db_orig}
\end{figure}
\begin{figure}\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.12\textwidth]{images/db.eps}}
...
\\\\
\begin{figure}[h]\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{images/ulab_sender.eps}}
...
V~takovéto konfiguraci prezentované obrázkem, který se interpretuje odshora dolů, je patrných pět nahrávacích zařízení, které jsou reprezentovány první řádkou uzlů. Mezi těmito zařízeními jsou zleva: obrazovka počítače (screen), webkamera (webcam), dvě kamery připojené přes rozhraní HDMI (hdmi0 a~hdmi1) a~nakonec jeden mikrofon (audio\_in). Z~kamer putují datové rámce do modulů pro upravení FPS (uzly fps0 -- fps2), dále všechny rámce putují do modulu \uv{delay}, který upraví zpoždění jednotlivých streamů dle potřeby. Obrazové streamy pak navštíví modul \uv{jpeg\_encoder} pro kompresi do formátu JPEG, čímž se zefektivní přenos dat po síti a~nakonec je všech pět streamů modulem \uv{uv\_rtp\_sender} vysláno sítí směrem k~cílové IP adrese a~portu, kde tato data přebírá jiná instance knihovny Libyuri (například na multimediálním serveru nebo v~infrastruktuře SAGE). 
...
\\
\begin{figure}[h]\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{images/ulab_receiver.eps}}
	\caption{Libyuri - konfigurace zobrazení v~SAGE a~ukládání}\label{img:navrh_backend_libyuri_receiver}
\end{figure}

Konfigurace na obrázku \ref{img:navrh_backend_libyuri_receiver} má podobnou strukturu, jako předchozí konfigurace. Opět se interpretuje odshora dolů. V~první řadě je patrných pět uzlů (uzly rtp\_video0 až rtp\_video3 a~rtp\_audio\_in), které se starají o~síťový příjem dat, které byly předchozí konfigurací odesílány pomocí RTP streamů. Obrazová data se ze síťových přijímačů posílají do modulů \uv{convert}, které provedou konverzi obrazů do shodného prostoru barev, dále se pomocí modulu \uv{scale} zmenší či zvětší na požadované rozlišení a~všechny se poté spojí vedle sebe do obrazové mřížky v~jediném modulu \uv{combine}. Z~tohoto modulu dále postupuje pouze jeden stream (který je kombinací všech čtyř obrazových dat) do modulu \uv{render\_text}, který na každý obrazový rámec přidá libovolný text (v~tomto případě časovou značku) a~nakonec jdou data do modulu \uv{dup}, který stream zduplikuje a~jeden z~nich pošle do zařízení SAGE (modul \uv{sage\_output}) a~druhý po konverzi do formátu JPEG a~upravení FPS uloží do souborového systému pomocí modulu \uv{filedump}. Nezmíněná pravá část obrázku \ref{img:navrh_backend_libyuri_receiver}, která zajišťuje zvuková data začíná taktéž uzlem pro příjem RTP streamu. Z~něho pak audio data postupují do již zmíněného modulu \uv{dup} a~po duplikaci se jeden stream posílá na výstup zvukového zařízení (v~tomto případě pomocí modulu \uv{jack\_output}) a~druhý se ukládá do souboru modulem \uv{wav\_dump}.
...
Prvních pár řádků konfigurace \uv{deploy job} se týká nastavení nutného pro výběr obrazu systému, zařazení jobu do správné etapy a~podobně. Sekce \uv{before\_script} nastavuje vzdálené připojení (pomocí asymetrické kryptografie) na privátní virtuální server pro nasazení aplikace. V~sekci \uv{script} jsou již uvedeny pouze čtyři příkazové řádky, které provedou nasazení softwaru na vzdáleném serveru. Nasazení proběhne tak, že se pomocí programu \textit{fpm} vytvoří balíček určený pro distribuce operačních systémů rodiny Debian. Tento balíček se následně přesune na požadovaný server a~za pomoci poslední řádky v~sekci \uv{script} a~skriptů obsažených v~balíčku se nasadí. Nakonec je uvedena sekce \uv{artifacts}, která na serveru kontinuální integrace uchová vytvořené balíčky pro různou potřebu, po dobu třiceti dnů.
...
Jak je patrné, objekt obsahuje název projektu, zdrojový soubor určený k~úpravě a~cílový soubor, do kterého se upravený soubor uloží. Krom možných dalších parametrů obsahuje ještě část \uv{actions}, která obsahuje seznam úprav, které Libyuri na zdrojovém souboru provede. V~dalším úryvku kódu je ukázáno, jak probíhá zpracování přijatého konfiguračního objektu na přijímací straně websocketů.\\
...
Po úspěšné instalaci programu \textit{FPM} se již může spustit skript pro vytvoření balíčku aplikace, pomocí skriptu \textit{makedeb.sh} v~adresáři \textit{scripts}, z~kořenového adresáře aplikace. Poté co skript skončí by se měl vytvořit soubor s~příponou \uv{.deb}, který reprezentuje zmiňovaný balíček.
...
\begin{figure}[h]\centering
	\includegraphics[width=1\paperwidth, angle=90]{images/ui_wf_ulab-editor-export.eps}
...
\begin{figure}
	\dirtree{%
		...
		.3 web\DTcomment{zdrojové kódy webové aplikace}.
		.3 yuri\_wrapper\DTcomment{zdrojové kódy aplikace obalující Libyuri v~CHUL}.
		.2 thesis\DTcomment{zdrojová forma práce ve formátu \LaTeX{}}.
		.3 images\DTcomment{obrazové soubory použité v~textu práce}.
		.3 resources\DTcomment{zdrojové soubory obrazových souborů}.
